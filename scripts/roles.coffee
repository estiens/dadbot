  ...
  formatUserRoles = (user) ->
    ...
    if user.roles.length > 0
      ...
      if user.roles.join('').search(',') > -1
        joiner = '; '
      "#{name} is #{user.roles.join(joiner)}."
    else
      "#{name} is nothing to me."

  robot.respond /who (?:am|is|are) @?([\w .\-]+)\?*$/i, (msg) ->
    ...
    if name is "you"
      ...
      msg.send "The best."
    else if name is "I" or name is "i"
      user = msg.message.user
      if user
        msg.send formatUserRoles user
      else
        msg.send "#{name}? Never heard of 'em"
    else
      users = robot.brain.usersForFuzzyName(name)
      if users.length is 1
        msg.send formatUserRoles users[0]
